...
import com.test.mybatis_accessor.dto.ProductOrderDto2;
...
public class Test04_UpdateEventExecutor4 {
    ...
    public void test1() throws MybatisAccessorException {
        ...
        int createUser = 5920;
        ProductOrderDto2 queryDto = new ProductOrderDto2();
        queryDto.setOrderName(testName);
        List<ProductOrderDto2> dtoList = mybatisAccessorService.getDtoListByAnnotation(queryDto);
        if(dtoList != null && dtoList.size() > 0) {
            ...
            // 验证主项被删除
            List<ProductOrderDto2> dtoListVerify = mybatisAccessorService.getDtoListByAnnotation(queryDto);
            assert(dtoListVerify == null || dtoListVerify.size() == 0);
        }

        ProductOrderDto2 orderDto = ProductOrderDto2.builder().orderName("a1").amount(amount)
        ...
        assert(po.getOrderDetails().stream().map(x -> x.getWeight()).reduce((x, y) -> x.add(y)).get().compareTo(po.getTotalWeight()) == 0);
